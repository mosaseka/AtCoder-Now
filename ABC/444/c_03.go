...
func main() {
	...
	sqrtSum := int64(math.Sqrt(float64(SUM)))

	for i := int64(1); i <= sqrtSum; i++ {
		if SUM%i == 0 {
			if i >= A_MAX {
				if checkL(i, BASE_COUNT) {
...
func checkL(L int64, BASE_COUNT map[int64]int64) bool {
