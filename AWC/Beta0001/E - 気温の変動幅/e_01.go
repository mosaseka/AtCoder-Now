...
func main() {
	...
	for i := 0; i < N; i++ {
		...
		for len(maxDeque) > 0 && H_LIST[maxDeque[len(maxDeque)-1]] <= h {
			maxDeque = maxDeque[:len(maxDeque)-1]
		}
		maxDeque = append(maxDeque, i)

		for len(minDeque) > 0 && H_LIST[minDeque[len(minDeque)-1]] >= h {
			minDeque = minDeque[:len(minDeque)-1]
		}
		minDeque = append(minDeque, i)

		if len(maxDeque) > 0 && maxDeque[0] <= i-K {
			maxDeque = maxDeque[1:]
		}
		if len(minDeque) > 0 && minDeque[0] <= i-K {
			minDeque = minDeque[1:]
		}

		if i >= K-1 {
			diff := H_LIST[maxDeque[0]] - H_LIST[minDeque[0]]
			if i == K-1 || diff > best {
				best = diff
			}
		}
